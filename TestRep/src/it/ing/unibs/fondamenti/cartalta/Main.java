...
public class Main 
{
	...
	private final static String PUNTA="Scegliere una quantità da puntare ";
	...
	private final static String CASE_STAY="1";
	private final static String CASE_EXIT="2";
	private final static String CASE="Per Uscire premere "+CASE_EXIT+",altrimenti "+CASE_EXIT+"per restare";
	...
	public static void loop(Giocatore ply)
	{
		do
		{
			...
			if(prima.getPunteggio()>seconda.getPunteggio())
			{
			...
			IO.out(CASE);
		}while(!IO.inKeyBoard(true).equalsIgnoreCase(CASE_EXIT));
